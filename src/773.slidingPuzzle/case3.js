...
var slidingPuzzle = function(board) {
    ...
    while (queue.length) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            for (const newStatus of getNewStatus(status, neighbors)) {
                if (check.has(newStatus)) {
                    continue;
                }
                if (newStatus === target) {
                    return steps;
...
function getNewStatus(status, neighbors) {
    const result = [];
    const zeroIndex = status.indexOf(0);
    const array = Array.from(status);

    for (const neighborIndex of neighbors[zeroIndex]) {
        [array[zeroIndex], array[neighborIndex]] = [array[neighborIndex], array[zeroIndex]];
