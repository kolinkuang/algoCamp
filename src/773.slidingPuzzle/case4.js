...
var slidingPuzzle = function(board) {
    ...
    while (queue.length) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            const cur = queue.pop();
            // Notice!
            for (const status of getNext(cur, neighbors)) {
                if (check.has(status)) {
                    continue;
                }
                if (status === target) {
...
function getNext(status, neighbors) {
    const result = [];

    const zeroIndex = status.indexOf(0);
    const array = Array.from(status);

    for (const pos of neighbors[zeroIndex]) {
        [array[pos], array[zeroIndex]] = [array[zeroIndex], array[pos]];
        // Notice!
