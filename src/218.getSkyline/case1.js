...
var getSkyline = function (buildings) {
    // 优先队列！
    const priorityQueue = [];
    const boundaries = buildings.flatMap(([x, y]) => ([x, y])).sort((a, b) => a - b);
    ...
    for (const boundary of boundaries) {
        while (index < n && buildings[index][0] <= boundary) {
            priorityQueue.push([buildings[index][1], buildings[index][2]]);
            index++;
        }
        while (priorityQueue.length && priorityQueue[0][0] <= boundary) {
            priorityQueue.shift();
        }

        const maxn = !priorityQueue.length ? 0 : priorityQueue[0][1];
        if (!result.length || maxn !== result[result.length - 1][1]) {
            result.push([boundary, maxn]);
        }
    }
