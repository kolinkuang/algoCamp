...
class Node {
    ...
    searchByName(descendantName) {
        if (this.name === descendantName) {
            return this;
        }

        for (const child of this.children) {
            const result = child.searchByName(descendantName);
            if (result) {
                return result;
            }
        }
    ...
    enrichInheritanceList(resultList) {
        resultList.push(this);

        for (const child of this.children) {
            child.enrichInheritanceList(resultList);
        }
    }
...
ThroneInheritance.prototype.birth = function(parentName, childName) {
    ...
    const child = parent.addChild(childName);
...
ThroneInheritance.prototype.getInheritanceOrder = function() {
    ...
    this.root.enrichInheritanceList(curOrder);

    return curOrder.filter(node => node.isLive()).map(node => node.name);
};
