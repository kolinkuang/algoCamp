...
var displayTable = function (orders) {
    ...
    for (const order of orders) {
        const table = order[1];
        const subMap = map.get(table) || new Map();
        const food = order[2];
        set.add(food);
        subMap.set(food, (subMap.get(food) || 0) + 1);
        map.set(table, subMap);
    }

    const result = [['Table', ...[...set].sort()]];

    for (const [table, subMap] of [...map].sort((a, b) => a[0] - b[0])) {
        const newLine = [table];
        for (const food of set) {
            const count = subMap.get(food) || '0';
            newLine.push('' + count);
        }
        result.push(newLine);
    }
