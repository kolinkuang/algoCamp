...
var canEat = function(candiesCount, queries) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        // console.log(`favoriteType = ${favoriteType}, favoriteDay = ${favoriteDay}, dailyCap = ${dailyCap}`);
        const x1 = favoriteDay + 1;
        const y1 = (favoriteDay + 1) * dailyCap;
        const x2 = favoriteType ? sum[favoriteType - 1] + 1 : 1;
        const y2 = sum[favoriteType];
        // console.log(`x1 = ${x2}, y1 = ${y1}, x2 = ${x2}, y2 = ${y2}`);
...
// 时间复杂度：O(m + n)
// 空间复杂度：O(m)
// m 为 candiesCount.length, n 为 queries.length
